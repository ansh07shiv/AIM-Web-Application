...
const StudentReportLateProjects = (props) => {
    ...
    const getResultsHandler = () => {
        axios({
            method: "POST",
            url: "http://127.0.0.1:5000/studentReportLateSubmission",
            data: {
                token,
                userType: "admin",
            }
                            ...
                            <Grid item xs={6}>
                                <List sx={{ bgcolor: 'background.paper' }}>
                                    {studentData.map((item) => (
                                        <>
                                            <StudentItem firstName={item.firstName} lastName={item.lastName} degree={item.degree} major={item.major}
                                                         grades={item.grades} uploadedDate={item.uploadedDate} isLateSubmit={item.isLateSubmit}/>
                                            <br/>
                                        </>
                                    ))}
                                </List>
...
export default StudentReportLateProjects;